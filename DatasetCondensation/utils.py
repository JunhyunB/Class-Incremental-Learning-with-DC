...
def get_dataset(dataset, data_path):
    if dataset == 'MNIST':
        ...
        dst_train = datasets.MNIST(data_path, train=True, download=True, transform=transform) # no augmentation
        idx = (dst_train.targets==0) | (dst_train.targets==1)
        dst_train.targets = dst_train.targets[idx]
        dst_train.data = dst_train.data[idx]

        dst_test = datasets.MNIST(data_path, train=False, download=True, transform=transform)
        idx = (dst_test.targets==0) | (dst_test.targets==1)
        dst_test.targets = dst_test.targets[idx]
        ...
        dst_train = datasets.CIFAR10(data_path, train=True, download=True, transform=transform) # no augmentation
        idx = (np.array(dst_train.targets) == 0) | (np.array(dst_train.targets) == 1)
        dst_train.targets = list(np.array(dst_train.targets)[idx])
        dst_train.data = dst_train.data[idx]

        dst_test = datasets.CIFAR10(data_path, train=False, download=True, transform=transform)
        idx = (np.array(dst_test.targets) == 0) | (np.array(dst_test.targets) == 1)
        dst_test.targets = list(np.array(dst_test.targets)[idx])
        dst_test.data = dst_test.data[idx]
        class_names = dst_train.classes
